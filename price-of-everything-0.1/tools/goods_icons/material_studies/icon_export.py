...
def export(raw: str, out: str, size: int = 800, contour: float = 0.014, margin: float = 0.04,
           strength: float = 0.42, outline_straps: bool = True):
    ...
    if outline_straps and strap.sum() > 400:
    ...
    if os.path.exists(id_path):
        ...
        ida = idc[..., 3] > 120
        # Decode all seven ID colours: argmax aliases yellow/cyan/magenta/white to RGB.
        bits = idc[..., :3] > 127
        lab = bits[..., 0].astype(np.uint8) + 2*bits[..., 1] + 4*bits[..., 2]
        lab[~ida] = 0
