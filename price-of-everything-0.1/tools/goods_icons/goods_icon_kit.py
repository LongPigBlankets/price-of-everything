...
def build_diesel_car():
    ...
    specs = [
        (-1.88, .60, .29, .48, .62, .43, .67, .72),
        (-1.77, .78, .23, .57, .73, .58, .77, .83),
        (-1.34, .89, .20, .62, .84, .70, .89, .96),
        (-0.82, .92, .20, .64, .91, .72, .97, 1.04),
        (-0.68, .92, .20, .65, .96, .68, 1.29, 1.43),
        (-0.39, .91, .20, .66, .98, .62, 1.50, 1.61),
        ( 0.22, .91, .20, .66, .98, .61, 1.55, 1.66),
        ( 1.10, .91, .20, .66, .98, .61, 1.55, 1.66),
        ( 1.72, .90, .21, .66, .97, .62, 1.52, 1.64),
        ( 2.22, .88, .22, .66, .95, .63, 1.47, 1.59),
    ...
    for axle, yw in enumerate(wheel_y):
        cutter = K.cyl("wheelwell_cutter_%d" % axle, 0, yw, R,
                       0.50 * D, 2.6 * D, navy, axis='X', segments=48, smooth=True)
        col.objects.unlink(cutter); helpers.objects.link(cutter)
        cutter.hide_render = True
        cutter.display_type = 'WIRE'
    ...
    # Glazing is fitted to the cage, leaving a substantial upright rear quarter.
    surface_panel("windscreen_left",
                  [(-0.68, -0.900, 1.04), (0.00, -0.925, 1.12),
                   (0.00, -0.675, 1.58), (-0.58, -0.650, 1.49)],
                  navy, 0.080, fine=False)
    surface_panel("windscreen_right",
                  [(0.00, -0.925, 1.12), (0.68, -0.900, 1.04),
                   (0.58, -0.650, 1.49), (0.00, -0.675, 1.58)],
                  navy, 0.080, fine=False)
    surface_panel("front_side_window",
                  [(0.895, -0.64, 0.99), (0.650, -0.36, 1.48),
                   (0.625, 0.43, 1.54), (0.902, 0.43, 0.99)],
                  navy, 0.020, fine=False)
    surface_panel("rear_side_window",
                  [(0.904, 0.51, 0.99), (0.625, 0.51, 1.54),
                   (0.635, 1.75, 1.50), (0.683, 2.23, 1.32),
                   (0.860, 2.34, 0.99)],
                  navy, 0.020, fine=False)

    # Shaped doors are coherent surface patches: sloped tops, kicked-up wheel corners and
    # a small crown make them read as pressed bodywork rather than vertical divider bars.
    surface_panel("front_door",
                  [(0.923, -0.61, 0.94), (0.928, 0.44, 0.96),
                   (0.934, 0.44, 0.36), (0.933, -0.27, 0.33),
                   (0.928, -0.48, 0.41), (0.925, -0.62, 0.58)],
                  body, 0.012, fine=True)
    surface_panel("rear_door",
                  [(0.930, 0.50, 0.96), (0.926, 1.67, 0.94),
                   (0.922, 1.72, 0.72), (0.924, 1.63, 0.54),
                   (0.928, 1.45, 0.39),
                   (0.936, 0.51, 0.35)], body, 0.012, fine=True)

    old_fine = K._fine_mode
    K._fine_mode = True
    K.sweep("shoulder_crease", [(0.930, y, z) for y, z in
                                ((-0.66, .91), (.20, .96), (1.30, .95), (2.42, .89))],
            0.010, silver, seg=12)
    K.sweep("rocker_crease", [(0.934, y, z) for y, z in
                              ((-.46, .36), (.42, .30), (1.02, .30), (1.40, .40))],
            0.010, body_dark, seg=12)
    for i, (yh, zh) in enumerate(((0.13, .88), (1.12, .87))):
        ...
        surface_panel("door_handle_inner_%d" % i,
                      ...
                      body, .012, fine=True)
    for side in (-1, 1):
        K.sweep("bonnet_seam_%s" % side,
                [(side * x, y, z) for x, y, z in
                 ((.42, -1.78, .78), (.49, -1.35, .93), (.52, -.90, 1.01),
                  (.50, -.80, 1.04))], 0.010, navy, seg=12)
    K._fine_mode = old_fine

    # The fascia is flush with the rounded nose.  Trapezoids follow its taper; no bumper,
    # lamp or grille box projects as a separate rectangular appliance.
    surface_panel("front_grille",
                  [(-.38, -1.895, .43), (.38, -1.895, .43),
                   (.32, -1.912, .56), (-.32, -1.912, .56)],
                  body_dark, 0.010, fine=True)
    for side in (-1, 1):
        surface_panel("headlamp_frame_%s" % side,
                      [(side * .73, -1.860, .53), (side * .37, -1.905, .57),
                       (side * .36, -1.900, .77), (side * .68, -1.855, .73)],
                      body_dark, 0.012, fine=True)
        surface_panel("headlamp_insert_%s" % side,
                      [(side * .67, -1.872, .57), (side * .42, -1.908, .60),
                       (side * .41, -1.903, .72), (side * .63, -1.868, .70)],
                      body, 0.010, fine=True)
        K._fine_mode = True
        for k, xoff in enumerate((.48, .59)):
            K.cyl("headlamp_bulb_%s_%d" % (side, k), side * xoff, -1.920, .650,
                  .043, .016, silver, axis='Y', segments=32, smooth=True)
        K._fine_mode = old_fine
    K._fine_mode = True
    K.sweep("integrated_bumper",
            [(-.70, -1.878, .34), (-.38, -1.930, .29),
             (.00, -1.945, .27), (.38, -1.930, .29), (.70, -1.878, .34)],
            0.018, body_dark, seg=12)
    K._fine_mode = old_fine

    # Wheels are slightly proud, with a clean five-hole steel rim.
    for axle, yw in enumerate(wheel_y):
        xw = 0.925
        arch = [(0.920, yw + math.cos(math.pi * k / 30) * 0.495,
                 R + math.sin(math.pi * k / 30) * 0.495) for k in range(31)]
        K._fine_mode = True
        K.sweep("fender_lip_%d" % axle, arch, 0.015, body_dark, seg=12)
        K._fine_mode = old_fine
        K.cyl("tyre_%d" % axle, xw, yw, R, R, 0.24, navy,
              axis='X', segments=48, smooth=True)
        K.cyl("sidewall_%d" % axle, xw + .13, yw, R, .375, .055,
              body_dark, axis='X', segments=48, smooth=True)
        K.cyl("rim_%d" % axle, xw + .165, yw, R, .270, .030,
    ...
    # Jerry can: local construction parented to a +28 degree yaw.  This brings the broad
    # stamped face toward the camera and reduces the visible side strip to the reference.
    can_root = bpy.data.objects.new("jerrycan_yaw", None)
    helpers.objects.link(can_root)
    can_root.location = (2.72, -1.63, 0)
    can_root.rotation_euler[2] = math.radians(8)
    can_root.scale.z = 1.15
    can_objects = []

    def keep(ob):
        ob.parent = can_root
        can_objects.append(ob)
        return ob

    can_profile = [(-.38, .05), (-.44, .13), (-.44, .91), (-.39, 1.04),
                   (-.25, 1.16), (.15, 1.16), (.31, 1.05), (.43, .91),
                   (.43, .13), (.37, .05)]
    depth = .37
    can = keep(loft_local("jerrycan_body",
                          [(-depth / 2, can_profile),
                           ( depth / 2, [(x * .97, z) for x, z in can_profile])],
                          red, smooth=False))
    bevel = can.modifiers.new("rolled_corners", 'BEVEL')
    bevel.width = .040; bevel.segments = 3

    K._fine_mode = True
    seam = [(x, -depth / 2 - .025, z) for x, z in can_profile]
    seam.append(seam[0])
    keep(K.sweep("jerry_rolled_seam", seam, .014, red_lo, seg=12))

    # Five narrow pressed ribs meet at one hub.  There is no heavy rectangular border.
    rib_y = -depth / 2 - .048
    hub = (0.01, rib_y, .57)
    for i, endpoint in enumerate(((-.25, rib_y, .84), (.25, rib_y, .84),
                                  (-.23, rib_y, .29), (.23, rib_y, .31),
                                  (.01, rib_y, .22))):
        keep(K.sweep("jerry_press_%d" % i, [endpoint, hub], .0065, red_lo, seg=10))

    # Single angular ring, modelled as one aperture.  Fine Freestyle applies to its inner
    # edge while the exporter's synthetic contour keeps the outside silhouette bold.
    outer = [(-.31, 1.10), (-.24, 1.39), (-.03, 1.50),
             (.30, 1.36), (.24, 1.13), (.06, 1.08)]
    inner = [(-.21, 1.255), (-.17, 1.345), (-.03, 1.395),
             (.20, 1.325), (.17, 1.275), (.04, 1.245)]
    keep(ring_local("jerry_handle", outer, inner,
                    -depth / 2 - .058, -depth / 2 + .055, red))
    keep(K.prism("jerry_handle_void", (0, -depth / 2 - .070, 0), (1, 0),
                 inner, .014, navy))
    K._fine_mode = old_fine

    # Cap sits on the opposite shoulder from the handle's high point, like the reference.
    keep(K.cyl("fuel_neck", -.36, -.23, 1.17, .105, .10,
               red_lo, axis='Z', segments=48, smooth=True))
    keep(K.cyl("fuel_cap", -.36, -.23, 1.25, .087, .045,
               red, axis='Z', segments=48, smooth=True))

    print("\n".join(K.validate(ground=0.0)))
    return {"objects": len(col.objects), "approach": "unified_subdivision_cage"}
...
def build_reference_jerrycan(K, col, origin=(0, 0, 0), yaw_deg=86.0, D=1.0,
    ...
    # Fresh real-can approach: a single rounded cage, not a side silhouette extruded into
    # a slab.  Lower support rings preserve the rolled bottom; upper rings shrink into a
    # one-third-width top deck and generate continuous compound shoulders.
    depth = .370
    cage_sections = [
        (.055, .000, .380, .145),
        (.075, .000, .410, .165),
    ...
    body = rounded_body_cage("body", cage_sections, red)
    ...
    # The game reference simplifies the real pressed handles into two broad polygonal
    # frames.  Model them as actual annular cage plates instead of tubes: the long flat
    # grip and rounded shoulders read clearly, while the legs descend far enough into the
    # top shell that their connection points are completely occluded.
    handle_red = toon_mat("tn_diesel_handle", (0.385, 0.068, 0.095))
    handle_outer = [
        (-.120, 1.060), (-.120, 1.070), (-.115, 1.083), (-.100, 1.108),
        (-.080, 1.133), (-.050, 1.150), (.050, 1.150), (.080, 1.133),
        (.100, 1.108), (.115, 1.083), (.120, 1.070), (.120, 1.060),
    ]
    handle_inner = [
        (-.090, 1.085), (-.090, 1.090), (-.084, 1.100), (-.070, 1.110),
        (-.050, 1.120), (-.030, 1.125), (.030, 1.125), (.050, 1.120),
        (.070, 1.110), (.084, 1.100), (.090, 1.090), (.090, 1.085),
    ]
    aperture_profile = [(-.085, 1.088), (-.080, 1.098), (-.066, 1.107),
                        (-.046, 1.116), (.046, 1.116), (.066, 1.107),
                        (.080, 1.098), (.085, 1.088)]
    for i, x in enumerate((-.035, .095)):
        handle = planar_shape("handle_cage_%d" % i, handle_outer,
                              (x, 0, (.005 if i else 0.0)),
                              (0, 1, 0), (0, 0, 1), (1, 0, 0),
                              -.021, .021, handle_red, inner=handle_inner)
        # Do not let automatic intersection/crease selection invent splinters at the
        # buried feet.  The aperture supplies the inner edge; one authored upper stroke
        # supplies the outer edge, exactly like the posterised game reference.
        no_ink(handle)
        outline_points = [(x, y, z + (.005 if i else 0.0))
                          for y, z in handle_outer[2:10]]
        outline = keep(K.sweep(prefix + "_handle_outline_%d" % i,
                               outline_points, .0055, navy, seg=8))
        no_ink(outline)
        aperture = planar_shape("handle_void_%d" % i, aperture_profile,
                                (x, 0, (.005 if i else 0.0)),
                                (0, 1, 0), (0, 0, 1), (1, 0, 0),
                                -.010, .010, navy)
        no_ink(aperture)
    ...
    # A dedicated 40-degree pad gives the cap one real plane rather than asking it to sit
    # on the cage's compound shoulder.  The pad is centred in depth and fully contains the
    # 20%-of-width cap footprint.
    cap_angle = math.radians(40.0)
    cap_axis = mathutils.Vector((-math.cos(cap_angle), 0.0,
                                 math.sin(cap_angle))).normalized()
    cap_tangent = mathutils.Vector((math.sin(cap_angle), 0.0,
                                    math.cos(cap_angle))).normalized()
    cap_base = mathutils.Vector((-.300, 0.0, 1.020))
    # The 270 x 270 mm-equivalent octagon is wider than the neck in both tangent axes.
    # Its deep back face replaces the compound cage surface under the fitting, so the
    # visible mounting patch is genuinely one plane rather than a wafer on a dome.
    pad = [(-.135, -.110), (-.110, -.135), (.110, -.135), (.135, -.110),
           (.135, .110), (.110, .135), (-.110, .135), (-.135, .110)]
    filler_pad = planar_shape("filler_pad", pad, cap_base,
                              (0, 1, 0), cap_tangent, cap_axis,
                              -.032, .008, red)
    no_ink(filler_pad)
    oriented_cyl("neck", cap_base + cap_axis * .025, .105, .080,
                 red_lo, cap_axis)
    oriented_cyl("cap", cap_base + cap_axis * .080, .092, .050,
                 red, cap_axis)
    oriented_cyl("cap_top", cap_base + cap_axis * .114, .076, .020,
                 red_lo, cap_axis)

    return {"root": root, "objects": made,
            "dimensions": {"width": .818 * D, "depth": depth * D,
                           "body_height": 1.214 * D, "total_height": 1.436 * D}}
