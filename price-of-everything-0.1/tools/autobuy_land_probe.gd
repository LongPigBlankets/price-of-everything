...
func _ready() -> void:
	...
	_check("OFF: no land bought", str(BuildingState.get_tile_land_owned(TILE)), "0")
	...
	var owned2 := BuildingState.get_tile_land_owned(TILE)
	var patches := int(ceil(float(need) / float(BuildingState.LAND_PATCH_SIZE)))
	_check("ON: build allowed", str(r2.get("allowed", false)), "true")
	_check("ON: bought exactly %d patch(es)" % patches, str(owned2), str(patches * BuildingState.LAND_PATCH_SIZE))
	_check("ON: covers the requirement", "yes" if owned2 >= need else "no", "yes")
	_check("ON: no over-buy (< one extra patch)", "yes" if owned2 - need < BuildingState.LAND_PATCH_SIZE else "no", "yes")
	print("[AUTOBUY] spent £%.2f on land" % (cash_before - MatchState.money))

	# 3) ON + already enough -> allowed, buys nothing more.
	_reset(need + BuildingState.LAND_PATCH_SIZE, 100000.0, true)
	...
	_check("ON+enough: land unchanged", str(BuildingState.get_tile_land_owned(TILE)), str(owned_before))
	...
	_check("ON+broke: no land bought", str(BuildingState.get_tile_land_owned(TILE)), "0")
...
func _reset(land: int, money: float, auto_buy: bool) -> void:
	BuildingState.tile_land_owned[TILE] = land
