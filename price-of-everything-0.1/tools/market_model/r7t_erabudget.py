...
def run(seed, player=False, BETA=0.5, ladder='standard', lag=None, entrant='standard',
        personas='balanced', demand='cyclical', apex_from=100, budget=None, era_budget=None):
    ...
    budget_left = (era_budget if era_budget is not None else
                   (budget if budget is not None else 10**9))
    ...
    for t in range(MAX_TURN + 1):
        if era_budget is not None and t in (101, 201):
            budget_left = era_budget
        for gid in traded:
...
print("=== PER-ERA ENTRANT BUDGET (refills at t101 and t201) ===")
for b in [10, 25, 50]:
    out["era_" + str(b)] = profile(f"  {b}/era (={b*3} total)", era_budget=b)
