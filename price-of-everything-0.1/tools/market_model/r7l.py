...
def run(seed, mode, player=False, BETA=0.5):
...
ORE = N("iron_ore")
out = {}
for beta in [0.50, 0.40, 0.60]:
    print(f"\n=== R7L beta {beta:.2f} ===")
    amb = [run(s, "r7e", BETA=beta) for s in seeds]
    st = msr(amb, STEEL)
    for name, gid in [("steel", STEEL), ("ev_car", EV)]:
        s_ = msr(amb, gid)
        s0 = [amb[0][0][gid][t] for t in range(MAX_TURN + 1)]
        nc, per, ampl = cycles(s0)
        print(f"  {name:<8}" + "".join(f"{s_[c]:>8.1f}" for c in CPS) + f"  | {nc} extrema, per {per:.0f}t, swing {ampl:.0f}pp, std {statistics.pstdev(s0[20:]):.1f}")
    ...
    print(f"  extrema {tot_ext} (h1 {h1} / h2 {h2}) | mean|imp| t300 {statistics.mean(abs(v) for v in at300):.1f}% | apex std {apex_std:.1f}pp")
    print(f"  events: {bk:.0f} bk, {acq:.0f} acq, {en:.0f} entrants")
    ply = [run(s, "r7e", player=True, BETA=beta) for s in seeds]
    imp_of = lambda g, t: statistics.mean(r[0][g][t] for r in ply)
    prof, cum, neg = pnl_integrated(imp_of)
    print(f"  integrated player: P&L £{cum:,.0f} ({100*cum/cum_base:.0f}%) | loss turns {neg}")
    out[f"b{beta:.2f}"] = {"steel_s0": [round(amb[0][0][STEEL][t], 2) for t in range(MAX_TURN + 1)],
                           "ev_s0": [round(amb[0][0][EV][t], 2) for t in range(MAX_TURN + 1)],
                           "pnl": round(100 * cum / cum_base), "ext": tot_ext}
json.dump(out, open(f"{ROOT}/r7l.json", "w"))
print("\nwrote r7l.json")
