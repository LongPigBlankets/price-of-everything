...
def run(seed, player=False, BETA=0.5, ladder='standard', lag=None, entrant='standard',
        personas='balanced', demand='cyclical', apex_from=100, budget=None, per_good=None):
    ...
    pg_left = defaultdict(lambda: (per_good if per_good is not None else 10**9))
    ...
    for t in range(MAX_TURN + 1):
        ...
        for gid in traded:
            ...
            if allow_entry and budget_left > 0 and pg_left[gid] > 0 and (hi10[gid] >= E_AT or hi20[gid] >= E_BT) and cd_en[gid] <= 0:
                ...
                if slot is not None:
                    ...
                    budget_left -= 1; pg_left[gid] -= 1; ent_by_tier[goods[gid]['goods_graph_tier']] += 1
                    ent_turns.append(t)
...
out = json.load(open(f"{ROOT}/r7s.json"))
NT = {t: len([g for g in impactable if goods[g]["goods_graph_tier"] == t])
      for t in ["raw","processed","intermediate","finished","apex"]}
print("goods per tier:", NT)
def tiers(label, **kw):
    amb = [run(s, **kw) for s in seeds]
    agg = defaultdict(float)
    for r in amb:
        for k, v in r[7].items(): agg[k] += v / len(amb)
    tot = sum(agg.values())
    apex_pct = 100 * agg.get("apex", 0) / max(tot, 1)
    cov = {t: f"{agg.get(t,0):.0f}/{NT[t]}" for t in NT}
    print(f"{label:<22} total {tot:5.0f} | apex {agg.get('apex',0):4.1f} ({apex_pct:4.1f}% of entry)"
          f" | per-tier {cov}")
    return {"total": round(tot), "apex": round(agg.get("apex",0),1), "apex_pct": round(apex_pct,1)}
print("=== does the cap protect apex entry? (ambient) ===")
res = {}
for pg in [1, 2, 3, 5]:
    res["pg" + str(pg)] = tiers(f"  per-good {pg}", per_good=pg)
for b in [50, 100, 150]:
    res["nat" + str(b)] = tiers(f"  national {b}", budget=b)
res["uncapped"] = tiers("  uncapped")
json.dump(res, open(f"{ROOT}/r7v.json", "w"))
print("wrote r7v.json")
