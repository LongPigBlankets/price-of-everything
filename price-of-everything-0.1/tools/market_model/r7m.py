...
def run(seed, player=False, BETA=0.5):
        ...
        for gid in traded:
            ...
            if allow_entry and (hi10[gid] >= 5 or hi20[gid] >= 2) and cd_en[gid] <= 0:
                ...
                if slot is not None:
                    ...
                    n_en += 1; cd_en[gid] = 10; hi10[gid] = 0; hi20[gid] = 0
                    if gid in APEX:
                        n_en_apex += 1
    ...
    return traj, n_bk, n_acq, n_en, n_en_apex, first_apex
...
print("=== R7M ambient (beta 0.50, apex competition era 2+, disruption window 12t) ===")
amb = [run(s) for s in seeds]
for name, gid in [("steel", STEEL), ("ev_car", EV), ("cpu", N("cpu"))]:
    s_ = msr(amb, gid)
    s0 = [amb[0][0][gid][t] for t in range(MAX_TURN + 1)]
    nc, per, ampl = cycles(s0)
    below = sum(1 for t in range(20, MAX_TURN + 1) if s0[t] < -2) / 281 * 100
    print(f"  {name:<8}" + "".join(f"{s_[c]:>8.1f}" for c in CPS) +
          f"  | {nc} ext, per {per:.0f}t, std {statistics.pstdev(s0[20:]):.1f}, below-base {below:.0f}%, min {min(s0):.1f}")
for tier in ["raw", "processed", "intermediate", "finished", "apex"]:
    tg = [g for g in impactable if goods[g]["goods_graph_tier"] == tier]
    s0s = [[amb[0][0][g][t] for t in range(20, MAX_TURN + 1)] for g in tg]
    below = statistics.mean(sum(1 for v in s if v < -2) / len(s) for s in s0s) * 100
    p10 = statistics.mean(sorted(s)[len(s)//10] for s in s0s)
    mins = statistics.mean(min(s) for s in s0s)
    print(f"    {tier:<13} below-base {below:4.0f}% of turns | p10 {p10:+5.1f}% | mean trough {mins:+5.1f}%")
tot_ext = sum(cycles([amb[0][0][gid][t] for t in range(MAX_TURN + 1)])[0] for gid in impactable)
bk = statistics.mean(r[1] for r in amb); acq = statistics.mean(r[2] for r in amb)
en = statistics.mean(r[3] for r in amb); ena = statistics.mean(r[4] for r in amb)
fa = [r[5] for r in amb if r[5] is not None]
at300 = [statistics.mean(r[0][gid][300] for r in amb) for gid in impactable]
print(f"  extrema {tot_ext} | mean|imp| t300 {statistics.mean(abs(v) for v in at300):.1f}%")
print(f"  events: {bk:.0f} bk, {acq:.0f} acq, {en:.0f} entrants ({ena:.0f} apex; first apex entry t{statistics.mean(fa):.0f})")

print("\n=== integrated player under R7M (EV competition arrives era 2) ===")
ply = [run(s, player=True) for s in seeds]
imp_of = lambda g, t: statistics.mean(r[0][g][t] for r in ply)
for name, gid in [("steel", STEEL), ("motor", MOT), ("ev", EV)]:
    s_ = msr(ply, gid)
    print(f"  {name:<6}" + "".join(f"{s_[c]:>8.1f}" for c in CPS))
cum, neg = pnl_integrated(imp_of)
print(f"  P&L £{cum:,.0f} ({100*cum/cum_base:.0f}%) | loss turns {neg}")

json.dump({"amb": {"steel_s0": [round(amb[0][0][STEEL][t], 2) for t in range(MAX_TURN + 1)],
                   "ev_s0": [round(amb[0][0][EV][t], 2) for t in range(MAX_TURN + 1)]},
           "player": {"ev": [round(x, 2) for x in msr(ply, EV)],
                      "motor": [round(x, 2) for x in msr(ply, MOT)]}},
          open(f"{ROOT}/r7m.json", "w"))
print("wrote r7m.json")
