...
APEXG = [g for g in impactable if goods[g]["goods_graph_tier"] == "apex"]
def apex_entrants(kw):
    amb = [run(s, **kw) for s in seeds]
    return statistics.mean(r[7].get("apex", 0) for r in amb)
print("=== PER-GOOD ENTRANT CAP (max entrants a single good can ever receive) ===")
for pg in [1, 2, 3, 5, 10]:
    out["pg_" + str(pg)] = profile(f"  {pg}/good", per_good=pg)
json.dump(out, open(f"{ROOT}/r7s.json", "w"))
print("wrote r7s.json (merged)")
