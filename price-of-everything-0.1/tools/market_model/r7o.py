...
CHAIN = ["iron_ore", "iron_ingots", "steel", "motor", "lithium_battery", "ev_car"]
...
# ---- ambient reference (no census prints) ----
amb = [run(s) for s in seeds]
census = []

# ---- 2. metal magnate sweep ----
WATCH = ["iron_ore", "iron_ingots", "steel", "copper_wiring", "motor", "lithium_carbonate",
         "graphite", "industrial_acids", "lithium_battery", "cpu", "ev_car"]
stage_res = {n: {} for n in WATCH}
pnls = {}
for k in [0, 5, 6]:
    label = "S0 ambient" if k == 0 else f"S{k}b +{CHAIN[k-1]}"
    if k == 0:
        runs = amb
    else:
        fl = make_player(k)
...
json.dump({"census": census,
          ...
          open(f"{ROOT}/r7o.json", "w"))
print("wrote r7o.json")
