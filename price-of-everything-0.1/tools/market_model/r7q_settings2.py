...
def run(seed, player=False, BETA=0.5, ladder='standard', lag=None, entrant='standard',
        personas='balanced', demand='cyclical'):
    ...
    for t in range(MAX_TURN + 1):
        ...
        for gid in traded:
            ...
            if allow_entry and (hi10[gid] >= E_AT or hi20[gid] >= E_BT) and cd_en[gid] <= 0:
...
def profile(label, **kw):
    ...
    steel_mean = statistics.mean(msr(amb, STEEL)[20:])
    print(f"{label:<26} apex_std {apex_std:5.1f} | midrange {rng:5.1f} | {above:3.0f}%up/{below:3.0f}%dn"
          f" | ext {ext:4d} | acq {acq:3.0f} ent {en:3.0f} | steel {steel_mean:+5.1f} | P&L {100*cum/cum_base:4.0f}% ({neg} loss)")
    return {"apex_std": round(apex_std,1), "mid_range": round(rng,1), "above": round(above), "below": round(below),
            "ext": ext, "acq": round(acq), "ent": round(en), "steel": round(steel_mean,1),
            "pnl": round(100*cum/cum_base), "neg": neg,
            "ev_s0": [round(amb[0][0][EV][t],2) for t in range(MAX_TURN+1)]}

out = json.load(open(f"{ROOT}/r7p.json"))
print("=== MARKET DEPTH (beta) ===")
for b in [0.40, 0.60]:
    out[f"beta_{b:.2f}"] = profile(f"  beta {b:.2f}", BETA=b)
print("=== RIVAL TEMPERAMENT (persona mix) ===")
for p in ["dramatic", "disciplined"]:
    out["pers_" + p] = profile(f"  {p}", personas=p)
print("=== NEW ENTRANTS (sensitivity) ===")
for e in ["eager", "patient"]:
    out["ent_" + e] = profile(f"  {e}", entrant=e)
print("=== WORLD DEMAND ===")
for d in ["flat", "boom"]:
    out["dem_" + d] = profile(f"  {d}", demand=d)
json.dump(out, open(f"{ROOT}/r7p.json", "w"))
print("wrote r7p.json (merged)")
