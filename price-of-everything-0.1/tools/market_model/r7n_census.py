#!/usr/bin/env python3
"""R7n — price-residence census on R7m (beta 0.50).

1. AMBIENT CENSUS: for every impacted good, % of turns (t20-300) above base
   (>+2%), below base (<-2%), near base, mean impact, trough, peak.
2. METAL MAGNATE INTEGRATION SWEEP: player enters at iron ore and integrates
   downstream, cumulative stages S1..S5:
     S1 ore -> S2 +ingots -> S3 +steel -> S4 +motors -> S5 +EVs
   One build/upgrade action per 5 turns ROTATING across active chains (fixed
   total attention: k chains => each steps every 5k turns; chain i starts
   t10+5i). Internal feeding down the chain; chain surpluses sold, shortfalls
   and all non-chain inputs bought. Residence table for every touched good at
   each stage + P&L.
"""
...
MAINT_L1 = 10.0
EXIT_HOLD, LAG, DELTA, DISRUPT = 10, 5, 0.0075, 12
BETA = 0.50
P333 = [0]*3 + [.5]*3 + [1]*3
...
CHAIN = ["iron_ore", "iron_ingots", "steel", "motor", "ev_car"]
CHAIN_IDS = [N(n) for n in CHAIN]
...
def demand_series(seed, gid):
    ...
    for t in range(MAX_TURN + 1):
        ...
        if ph == 20: dstep = -boom_log * rng.uniform(flo, fhi) / 10
...
def stage_levels(k):
    """k active chains; one action per 5 turns rotating; chain i starts t10+5i."""
    ...
    while t <= 300:
        c = idx % k
        hist.append((t, c, counts[c])); counts[c] += 1
        t += 5; idx += 1
    out = [[] for _ in range(k)]
    hi = 0
    cur = [list() for _ in range(k)]
    for t in range(MAX_TURN + 1):
        while hi < len(hist) and hist[hi][0] == t:
            _, c, si = hist[hi]; hi += 1
            if si % 3 == 0: cur[c].append(1)
            else: cur[c][-1] = min(3, cur[c][-1] + 1)
        for c in range(k):
            out[c].append(list(cur[c]))
    return out

def make_player(k):
    """Returns flows(t) -> (pnet, rev_units, buy_units) for stages CHAIN[:k]."""
    LV = stage_levels(k)
    specs = []
    for ci, name in enumerate(CHAIN[:k]):
        g = by_name[name]
        specs.append((g["id"], g["base_recipe_output_qty"],
                      [(i["good_id"], i["qty"]) for i in g["base_recipe_inputs"]],
                      g["base_recipe_energy"], ci))
    def flows(t):
        prod = {}; need = defaultdict(float)
        for gid, batch, ins, en, ci in specs:
            lv = LV[ci][t]
            prod[gid] = sum(batch * OUT_M[l] for l in lv)
            for jgid, qty in ins:
                need[jgid] += sum(qty * IN_M[l] for l in lv)
        pnet = defaultdict(float)
        for gid in prod: pnet[gid] += prod[gid]
        for jgid, amt in need.items(): pnet[jgid] -= amt
        return dict(pnet), prod, dict(need), specs, LV
    return flows

def run(seed, flows=None):
    ...
    Mdem = {gid: demand_series(seed, gid) for gid in impactable}
    ...
    for t in range(MAX_TURN + 1):
        for gid in traded:
            ...
            hi10[gid] = hi10[gid] + 1 if d_o >= 10 else 0
            hi20[gid] = hi20[gid] + 1 if d_o >= 20 else 0
        for gid in traded:
            ...
            if dis_cnt[gid] >= 10 and cd_acq[gid] <= 0:
                ...
                if len(cands) >= 2:
                    ...
                    if seller != buyer:
                        ...
                        cd_acq[gid] = 15; dis_cnt[gid] = 0
            if low_cnt[gid] >= 15:
                for i in range(len(active[gid])):
                    if active[gid][i] and q[gid][i] <= base * 1.05:
                        active[gid][i] = False; q[gid][i] = 0.0; low_cnt[gid] = 0
                        break
            allow_entry = (gid not in APEX) or (t > 100)
            if allow_entry and (hi10[gid] >= 5 or hi20[gid] >= 2) and cd_en[gid] <= 0:
                ...
                if slot is not None:
                    ...
                    cd_en[gid] = 10; hi10[gid] = 0; hi20[gid] = 0
        if t > 0 and t % STEP == 0:
            b = t // STEP - 1
            for gid in traded:
                ...
                for i in range(len(active[gid])):
                    ...
                    s = P333[i % R_CT] if (gid not in APEX and i < R_CT) else 1.0
                    if exiting and s > 0:
...
def pnl(flows, imp_of):
    cum = 0.0; neg = 0
    for t in range(MAX_TURN + 1):
        pnet, prod, need, specs, LV = flows(t)
        rev = cost = 0.0
...
def residence(series):
    s = series[20:]
    above = sum(1 for v in s if v > 2) * 100 // len(s)
    below = sum(1 for v in s if v < -2) * 100 // len(s)
    return above, below, round(statistics.mean(s), 1), round(min(s), 1), round(max(s), 1)

seeds = list(D["rivals"].keys())
def msr(runs, gid): return [statistics.mean(r[gid][t] for r in runs) for t in range(MAX_TURN + 1)]

# ---- 1. ambient census ----
amb = [run(s) for s in seeds]
census = []
for gid in impactable:
    a, b, m, lo, hi = residence(msr(amb, gid))
    census.append({"name": goods[gid]["internal_name"], "tier": goods[gid]["goods_graph_tier"],
                   "above": a, "below": b, "mean": m, "min": lo, "max": hi})
census.sort(key=lambda r: r["above"] - r["below"], reverse=True)
print("=== AMBIENT CENSUS (t20-300, above = >+2%, below = <-2%) ===")
print(f"{'good':<24}{'tier':<14}{'above%':>7}{'below%':>7}{'mean':>7}{'min':>7}{'max':>7}")
for r in census[:10]: print(f"{r['name']:<24}{r['tier']:<14}{r['above']:>7}{r['below']:>7}{r['mean']:>7}{r['min']:>7}{r['max']:>7}")
print("  ...")
for r in census[-10:]: print(f"{r['name']:<24}{r['tier']:<14}{r['above']:>7}{r['below']:>7}{r['mean']:>7}{r['min']:>7}{r['max']:>7}")
for tier in ["raw", "processed", "intermediate", "finished", "apex"]:
    tr = [r for r in census if r["tier"] == tier]
    print(f"  {tier:<13} avg above {statistics.mean(r['above'] for r in tr):3.0f}%  below {statistics.mean(r['below'] for r in tr):3.0f}%")

# ---- 2. metal magnate sweep ----
WATCH = ["iron_ore", "coal", "iron_ingots", "hydrogen", "steel", "copper_wiring", "motor",
         "car_body", "glass", "tyres", "lithium_battery", "cpu", "ev_car"]
stage_res = {n: {} for n in WATCH}
pnls = {}
for k in range(0, 6):
    label = "S0 ambient" if k == 0 else f"S{k} +{CHAIN[k-1]}"
    if k == 0:
        runs = amb
    else:
        fl = make_player(k)
        runs = [run(s, flows=fl) for s in seeds]
        imp_of = lambda g, t: statistics.mean(r[g][t] for r in runs)
        cum, neg = pnl(fl, imp_of)
        pnls[k] = (cum, neg)
    for n in WATCH:
        stage_res[n][k] = residence(msr(runs, by_name[n]["id"]))
    print(f"\n=== {label} ===" + (f"  P&L £{pnls[k][0]:,.0f}, loss turns {pnls[k][1]}" if k else ""))
    print(f"{'good':<16}" + f"{'above%':>8}{'below%':>8}{'mean':>7}")
    for n in WATCH:
        a, b, m, lo, hi = stage_res[n][k]
        mark = " <<" if k and n in CHAIN[:k] else ("  *" if k and any(
            n == goods[i["good_id"]]["internal_name"] for cn in CHAIN[:k] for i in by_name[cn]["base_recipe_inputs"]) else "")
        print(f"{n:<16}{a:>8}{b:>8}{m:>7}{mark}")

json.dump({"census": census,
           "stages": {n: {str(k): stage_res[n][k] for k in stage_res[n]} for n in WATCH},
           "pnl": {str(k): pnls[k][0] for k in pnls}},
          open(f"{ROOT}/r7n.json", "w"))
print("\nwrote r7n.json")
