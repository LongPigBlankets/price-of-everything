...
def run(seed, player=False, BETA=0.5, ladder='standard', lag=None):
...
def profile(label, **kw):
    ...
    steel = msr(amb, STEEL)
    print(f"{label:<28} apex_std {apex_std:5.1f}pp | mid range {rng:5.1f}pp | mid {above:3.0f}%up/{below:3.0f}%down"
          f" | ext {ext:4d} | acq {acq:3.0f} ent {en:3.0f} | P&L {100*cum/cum_base:3.0f}% ({neg} loss)")
    return {"label": label, "apex_std": round(apex_std, 1), "mid_range": round(rng, 1),
            "above": round(above), "below": round(below), "ext": ext,
            "acq": round(acq), "ent": round(en), "pnl": round(100 * cum / cum_base), "neg": neg,
            "steel": [round(x, 2) for x in steel],
            "ev_s0": [round(amb[0][0][EV][t], 2) for t in range(MAX_TURN + 1)]}

out = {}
print("=== PRICE VOLATILITY (ladder), lag 5, beta .50 ===")
for lad in ["steady", "standard", "turbulent"]:
    out["vol_" + lad] = profile(f"  volatility: {lad}", ladder=lad)
print("=== COMPETITOR REFLEXES (lag), standard ladder, beta .50 ===")
for lg in [3, 5, 10]:
    out["lag_" + str(lg)] = profile(f"  lag {lg} turns", lag=lg)
json.dump(out, open(f"{ROOT}/r7p.json", "w"))
print("wrote r7p.json")
