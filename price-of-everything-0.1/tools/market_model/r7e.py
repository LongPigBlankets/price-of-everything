...
FRAC_RANGES = {"raw": (0.70, 1.00), "processed": (0.60, 1.05), "intermediate": (0.50, 1.10),
               "finished": (0.45, 1.15), "apex": (0.40, 1.20)}
...
print("=== R7E ambient (fraction-drawn tiered sawtooth) ===")
amb = [run(s, "r7e") for s in seeds]
st = msr(amb, STEEL)
ORE = N("iron_ore")
...
for tier in ["raw", "processed", "intermediate", "finished", "apex"]:
    ...
    print(f"    {tier:<13} t300 {statistics.mean(vals):+6.1f}%  std {statistics.mean(vols):4.1f}pp  extrema/good {statistics.mean(exs):.1f}")
...
json.dump({"amb": {"steel": [round(x,2) for x in st],
          ...
          open(f"{ROOT}/r7e.json", "w"))
print("wrote r7e.json")
