...
def demand_wobble(seed, gid, mode):
    ...
    else: slots, gstep, dstep = ([1]*20 + [-1]*8 + [0]*12), 0.005, 0.0125
...
def run_r6(seed, mode, player=None):
    ...
    for t in range(MAX_TURN + 1):
        ...
        for gid, g in goods.items():
            ...
            dem = wob[gid][t] * BETA * sold.get(gid, 0) if gid in wob else 0.0
...
out = {}
for label, mode in [("R6a events-only", "none"), ("R6b +demand 5-2-3", "fast"), ("R6c +demand 20-8-12", "slow")]:
...
json.dump(out, open(f"{ROOT}/r6.json", "w"))
print("\nwrote r6.json")
