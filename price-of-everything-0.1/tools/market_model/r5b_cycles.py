...
def run_r5(seed, player=None):
    ...
    traj = defaultdict(list); n_bk = 0; n_en = 0; cooldown = defaultdict(int)
    def in_dev(gid):
    ...
    for t in range(MAX_TURN + 1):
        for gid in producible:
            cooldown[gid] = max(0, cooldown[gid] - 1)
            d_o = imp.get(gid, 0.0)
        ...
        for gid in producible:
            ...
            if hi_cnt[gid] >= EN_HOLD and cooldown[gid] <= 0:
                placed = False
                for i in range(len(active[gid])):
                    if not active[gid][i]:
                        active[gid][i] = True; q[gid][i] = float(base); placed = True
                        break
                if not placed and len(active[gid]) < MAX_SLOTS:
                    active[gid].append(True); q[gid].append(float(base))
                    inc[gid].append(inc[gid][len(active[gid]) % R_CT])
                    placed = True
                if placed:
                    n_en += 1; hi_cnt[gid] = 0; cooldown[gid] = EN_COOLDOWN
        if t > 0 and t % STEP == 0:
            b = t // STEP - 1
            for gid in producible:
...
print("R5b (lag 12, strong out-response, lumpy entry) ambient:")
...
json.dump({"amb_steel_b": [round(x,2) for x in st], "amb_solar_b": [round(x,2) for x in so],
           "amb_steel_s0": [round(x,2) for x in st1], "amb_solar_s0": [round(x,2) for x in so1],
           "spec_b": [round(x,2) for x in sp]},
          open(f"{ROOT}/r5b.json", "w"))
print("wrote r5b.json")
