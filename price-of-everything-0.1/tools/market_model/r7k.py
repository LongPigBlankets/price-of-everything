...
BETA, C_THR, SPEED, EMA_N = 0.75, 0.6, 2.5, 5
...
def run(seed, mode, player=False):
    ...
    for t in range(MAX_TURN + 1):
        for gid in producible:
            ...
            hi30[gid] = hi30[gid] + 1 if d_o >= 10 else 0
            hi40[gid] = hi40[gid] + 1 if d_o >= 20 else 0
        for gid in producible:
            ...
            if (hi30[gid] >= 5 or hi40[gid] >= 2) and cd_en[gid] <= 0:
                ...
                for i in range(len(active[gid])):
                    if not active[gid][i]:
                        active[gid][i] = True; q[gid][i] = size; placed = True; break
                if not placed and len(active[gid]) < 12:
                    active[gid].append(True); q[gid].append(size)
        ...
        for gid, g in goods.items():
            ...
            T = 9.0 * base   # R7i: static, anchored to INITIAL world production
            r = net / T
...
def dec_price(gid, t):
    g = goods[gid]
    if not DECAY_ON: return g["base_price"]
...
print("=== R7K: R7J + era-scaled entrants (e1: 1/2/3x, e2: 5/8/12x, e3: 15/25/40x base) ===")
amb = [run(s, "r7e") for s in seeds]  # r7e series gen, g params
st = msr(amb, STEEL)
ORE = N("iron_ore")
for name, gid in [("steel", STEEL), ("iron_ore", ORE), ("ev_car", EV)]:
    s_ = msr(amb, gid)
    s0 = [amb[0][0][gid][t] for t in range(MAX_TURN + 1)]
    nc, per, ampl = cycles(s0)
    vol = statistics.pstdev(s0[20:])
    print(f"  {name:<10}" + "".join(f"{s_[c]:>8.1f}" for c in CPS) + f"  | seed0: {nc} extrema, period {per:.0f}t, swing {ampl:.0f}pp, std {vol:.1f}")
for tier in ["raw", "processed", "intermediate", "finished", "apex"]:
    ...
    vols = [statistics.pstdev([amb[0][0][g][t] for t in range(20, MAX_TURN + 1)]) for g in tg]
    exs = [cycles([amb[0][0][g][t] for t in range(MAX_TURN + 1)])[0] for g in tg]
    rngs = [max(x) - min(x) for x in ([[amb[0][0][g][t] for t in range(20, MAX_TURN + 1)]] for g in tg) for x in [x[0]]]
    above = statistics.mean(sum(1 for t in range(20, MAX_TURN + 1) if amb[0][0][g][t] > 2) / 281 for g in tg)
    below = statistics.mean(sum(1 for t in range(20, MAX_TURN + 1) if amb[0][0][g][t] < -2) / 281 for g in tg)
    print(f"    {tier:<13} t300 {statistics.mean(vals):+6.1f}%  std {statistics.mean(vols):4.1f}pp  range {statistics.mean(rngs):4.0f}pp  extrema/good {statistics.mean(exs):.1f}  above/below base {above*100:.0f}%/{below*100:.0f}%")
tot_ext = sum(cycles([amb[0][0][gid][t] for t in range(MAX_TURN + 1)])[0] for gid in impactable)
h1 = sum(cycles([amb[0][0][gid][t] for t in range(151)] + [amb[0][0][gid][150]] * 150)[0] for gid in impactable)
h2 = sum(cycles([amb[0][0][gid][150]] * 150 + [amb[0][0][gid][t] for t in range(150, MAX_TURN + 1)])[0] for gid in impactable)
print(f"  rhythm split: extrema t0-150: {h1}, t150-300: {h2}")
bk = statistics.mean(r[1] for r in amb); acq = statistics.mean(r[2] for r in amb); en = statistics.mean(r[3] for r in amb)
demM = statistics.mean(demand_series(seeds[0], gid, "r7e")[300] for gid in producible)
at300 = [statistics.mean(r[0][gid][300] for r in amb) for gid in impactable]
print(f"  ALL-goods extrema seed0: {tot_ext} | mean|imp| t300 {statistics.mean(abs(v) for v in at300):.1f}% | demand mult t300 {demM:.2f}")
print(f"  events: {bk:.0f} bk, {acq:.0f} acq, {en:.0f} entrants")

print("\n=== integrated 3-chain player under R7E ===")
ply = [run(s, "r7e", player=True) for s in seeds]
imp_of = lambda g, t: statistics.mean(r[0][g][t] for r in ply)
stp, mop, evp = msr(ply, STEEL), msr(ply, MOT), msr(ply, EV)
for name, s_ in [("steel", stp), ("motor", mop), ("ev", evp)]:
    print(f"  {name:<6}" + "".join(f"{s_[c]:>8.1f}" for c in CPS))
nc, per, ampl = cycles([statistics.mean(r[0][EV][t] for r in ply) for t in range(MAX_TURN + 1)])
print(f"  EV cycles (mean traj): {nc} extrema, period {per:.0f}t, swing {ampl:.0f}pp")
_, cum_base, _ = pnl_integrated(lambda g, t: 0.0)
prof, cum, neg = pnl_integrated(imp_of)
print(f"  R7e (decay ON):  P&L £{cum:,.0f} ({100*cum/cum_base:.0f}% of £{cum_base:,.0f}) | loss turns {neg}")
DECAY_ON = False
globals()["DECAY_ON"] = False
_, cum_base_f, _ = pnl_integrated(lambda g, t: 0.0)
prof_f, cum_f, neg_f = pnl_integrated(imp_of)
print(f"  R7f (decay OFF): P&L £{cum_f:,.0f} ({100*cum_f/cum_base_f:.0f}% of £{cum_base_f:,.0f}) | loss turns {neg_f}")
print(f"  decay removal: baseline £{cum_base:,.0f} -> £{cum_base_f:,.0f} (x{cum_base_f/cum_base:.2f}); player x{cum_f/cum:.2f}")

json.dump({"amb": {"steel": [round(x,2) for x in st],
                   "steel_s0": [round(amb[0][0][STEEL][t],2) for t in range(MAX_TURN+1)],
                   "ev_s0": [round(amb[0][0][EV][t],2) for t in range(MAX_TURN+1)],
                   "ore_s0": [round(amb[0][0][ORE][t],2) for t in range(MAX_TURN+1)]},
           "player": {"steel": [round(x,2) for x in stp], "motor": [round(x,2) for x in mop],
                      "ev": [round(x,2) for x in evp]}},
          open(f"{ROOT}/r7k.json", "w"))
print("wrote r7k.json")
