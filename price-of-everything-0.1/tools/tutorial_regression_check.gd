...
func _run() -> void:
	...
	var dock := node_named("ToastLayer")
	dock.push_research("High Strength Glassmaking")
	check(dock.has_row("research:High Strength Glassmaking"), "tutorial keeps research unlocks in the updates dock")
	var rows_before: int = dock.row_count()
	notice_bar._evaluate_anomalies({}, {})
	check(dock.row_count() == rows_before, "tutorial suppresses money and power notices")
