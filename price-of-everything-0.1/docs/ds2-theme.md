...
| **Gauge set in a plate** | the panel gauge (bezel, zones, needle, LED) in a chamfered hole cut in the dark plate | `darkplate` → `gauge_socket`; gauge layers in `assets/ui/gauge/` | `PanelGauge` (`scripts/panel_gauge.gd`) held in a trimmed holder, which paints the socket under it | cost to produce |
...
- **A canvas blurred with `filter: blur()` fades its edges.** When `steelWindow` repeats it mirrored, the fade shows as a bright seam. Blur through `drawBlurred`, which mirror-pads the texture first, as every steel and brass canvas now does. The backing carried such a seam, a light band across the panel partway down, until it was re-rendered this way.
- **A control that paints under another from positions read at draw time goes stale.** When the other moves and the painter keeps its size, nothing redraws the painter. Paint in the moving control's own coordinates instead: each gauge's holder paints its socket.
