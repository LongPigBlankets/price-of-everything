...
Status: BUILT and the DEFAULT since 26 September 2026 (`toggle topbar ds2` switches back to v3.1; built 24 September 2026): the strip (navy steel, H-beam, concrete behind the centred cash, silver pipe loop), raised icons and pilot lamps, the cash on an LED, Victory on a drum counter, hover readouts, shared Power and Transport status, Treasury and Power as navy steel sheets, a Rankings panel, Victory and Council opening their panels, one text standard with Building Detail. The updates dock holds toasts, research, notices and decisions; the notch is gone. Still open: the mission to the dock as a work order (with its flyout), the turn as a drum counter, Power aligned with Building Detail's per-building checks, and a saved standard for the bar. The owner's decisions are in §10; the method is written up in `docs/ds2-theme.md` §13.
