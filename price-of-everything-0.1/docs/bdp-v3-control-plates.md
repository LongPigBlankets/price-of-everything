...
- **What it would do:** while a cover is lifted (`BdpV3Footer.cover_changed`), a steel plate (the action sheets' plate) slides up from behind the footer with what pressing the button would do, and slides back when the cover drops or the button is pressed. Sell: "Building will become NPC" and "You will receive" with the sale price (`BuildingPrice.sale_price`) on an LED screen after a £. Demolish: how much land it frees (`BuildingState.space_used`) and its refund (`BuildingWorks.refund_cost`), the goods as 50 px icons set in thin frames with their quantities, five to a row. The lifted cover stands in front of the plate.
...
- **Value added in production:** the output's value less inputs, labour and upkeep (maintenance, power, storage and the carbon levy). Its name is printed in navy on a wide worn off-white keycap with a chevron (`bdp_v3_mod_key.gd`, as the Modifiers key is), its figure on a screen beside it; pressing the key opens the row, and it latches down while open, showing each part on its own screen: Output (or Output (if sold)), Inputs (left out when they come free), Labour and Upkeep.
- **Transport costs:** bringing the inputs in and taking the output to market, on the same kind of key. It opens to Inputs and Outputs, each on a smaller key (the same keycap at 0.8, `key_scale`) with its side's total, and each of those opens to its goods: every good's freight by how it goes and its port charge on their own rows (Steel · Road, Steel · Port, Motor · Port, or the logistics intermediary).
- **Net Value Added:** the first less the second, before tax.

Which rows are open is kept while the panel rebuilds. Under them, the revenue and the costs as two bars on one scale (`bdp_v3_value_bar.gd`), each on a mini screen with its name printed beside it, so the gap between their ends is what the building adds. The revenue bar ("Revenue if sold" while the output stays in stock) has a slice for each good sold in shades of green, the good's own icon over it on a small rounded tile (the coins for a power plant's power); the cost bar has inputs, labour, upkeep and transport in shades of red, each with its raised icon (`econ_icon_<name>`: the INPUTS wheelbarrow, the engineer, gears, the lorry). A faint dashed mark on the cost bar shows where the revenue ends. Icons that crowd spread apart; a short line joins each icon to its slice. Hovering a slice names it with its £ and its share of the revenue. The money frame leaves a gap after the modifiers and after the economics.
...
In v3 the diagnostics section is a moulded dark plastic case (`plasticPlate`, `diag_plastic`, a 9-slice) instead of a steel frame, with silver cross-head screws (`screw_silver`) round its edge, 9 px in: four along the top and four along the bottom, corner to corner, and six down each side, counting the corners (`bdp_v3_section.gd`, `style = "plastic"`, `screw_points`). The heading has a Visual / Text slide switch beside it (`toggleSlot`, `toggleKnob`, `bdp_v3_toggle.gd`): a slot moulded into the case and an off-white ridged thumb that slides across when clicked. It is set to Text at first. Its side is the player's choice, kept in `UiPrefs.bdp_diag_visual` while the game runs, so closing the panel or opening another building keeps it. Thrown to Visual it shows the visual view instead of the rows: both are built, and the switch shows one. While a tutorial step spotlights the rows (`Tutorial.active_spotlight_ref()` is "DiagnosticsCard"), the rows show whatever the switch says, since the step's words describe them.

The visual view (`_build_v3_diag_visual`) is the chain left to right, a raised module per stage (Inputs, Inbound, Power, Plant, Outputs), its name in metal letters over its checks, one to a row. Icons are 56 px (the `diagicon` renders are 112 texels of art for it), at least 6 px in from their column's sides, their lamps 0.72, the size of the text rows' lamps. Every column is five rows of icons tall whatever it holds (`V3_DIAG_ROWS`, Outputs' five), so the columns stand level. An indicator over several goods shows a lamp per tone among them, worst first, at most three. Each check is an indicator (`bdp_v3_indicator.gd`), a raised icon over a lamp lit for its tone. Hovering one names it and its finding in the readout at the foot (`bdp_v3_readout.gd`), a dark glass screen in the LED screens' bezel; with the pointer on none, the readout shows the worst check. While the case's foot is below the fold the readout rises onto the scroll area's bottom edge, never over the first row (ds2-theme §7.11). Power is wired: `BuildingReadout.power_checks` gives Power supply (the power plant icon: own supply green, the grid or ready to draw amber, no connection red; a plant is its own source), Intermittency (a battery drawn for it, `batteryArt`: a solid body and terminal posts, a thick outline groove and a bold plus, bolt and minus cut through, nothing round it: tones from the checklist's intermittency row; grey power green; unlit before any power is drawn) and Cable capacity (the pylons icon: the tile's draw or output against its cable cap, amber from 90%, red when the cap blocked the building or there are no cables). A building that neither uses nor makes power has all three unlit. The icons are building icons raised by the `diagicon` render set.

Inputs is wired: `BuildingReadout.input_checks` gives Source (the control plate's Inputs icon, the wheelbarrow: a lamp for each input, green from the company's own buildings or the intermediary, amber bought at the market, red when the checklist calls the building starved and that input is short; unlit before anything is sourced), Stock cover (three stacked crates: the runs the tile's stock covers and the input that runs short first; amber when an input is short but arriving, or when the scarcest runs out before its delivery lands; red when the engine calls the building starved), Upstream health (the Building Ledger's factory with an arrow leaving it: the worst status of the company's own buildings routed to supply it, unlit when everything is bought) and Deposit left (a pickaxe, its head a crescent pointed at both ends: a mine's turns of deposit left, amber from the engine's running-out warning, red when exhausted, unlit for a building that mines nothing). The crates and pickaxe are drawn for it; the wheelbarrow is the game icon, trimmed, and the factory the Building Ledger's button icon keyed off its translucent button (`keyOpaque`) with a drawn arrow.

Inbound is wired: `BuildingReadout.inbound_checks` gives Route and mode (an S-shaped arrow: a lamp for each input, red with no route in, amber when a cheaper kind of infrastructure suits it than it comes by, as Outputs' Reach, green when it already comes the cheapest way or needs no travel), Warehouse room (a shed with a roller door: red when the tile's warehouse is full and the building is starved, amber when full or 90% full, else green with the room left), Transit time (a clock: the slowest input's route, from the market's quote or the nearest of the company's own producers; green within a turn, amber to four, red beyond, the same bands as the outputs' reach) and Freight cost (a lorry with a £ coin: the Economics section's input transport lamp and its figures, the cost a turn and its share of the inputs' value). All four icons are drawn for it. A building that takes no inputs has all four unlit.

Plant is wired: `BuildingReadout.plant_checks` gives Carbon levy (a chimney under a smoke cloud with a £ cut through it, drawn for it: green before the levy is in force or when the building burns nothing it taxes; amber while it pays, naming the taxed inputs; red when the levy alone turns the building's net value added from a profit into a loss) and Upgrade and retool works (a crossed hammer and spanner, drawn for it: unlit with no works; amber while upgrading, retooling or being demolished; red when an upgrade has stalled). The carbon check reads the economics the panel quotes once per rebuild for its Economics section. Cost against market and output against full rate are left out: the Cost to produce gauges and the recipe diagram say them.

Outputs is wired: `BuildingReadout.output_checks` works each check out for every good the recipe makes and combines them (`_combine_outputs`): its lamps are the distinct tones among the goods, worst first (at most red, amber, green), its detail the worst good's, naming it and counting the rest by colour. Reach (the S-shaped arrow) never goes red: amber when the good's route isn't all on the cheapest kind of infrastructure that suits it (`_cheaper_mode`: rail for a solid, pipeline for a fluid, the reinforced one where only that carries it; no cost is worked out), or when it can't reach its destination at all; green otherwise. Transit time (the clock: turns to the destination, green within a turn, amber to four, red beyond, and red when there is no route out) and Freight cost (the lorry with a £ coin: £ a unit, green under £0.15, amber from there) take the checklist's output rows' bands. Port charge (an anchor): the port's rate is set by the turn, so the lamp watches what the company's own volume does to it; it reads the port the output sells through (`TransportState.seaport_shipping_summary`): green with room, giving the rate and that owning the port halves it; amber when this turn's traffic of the good's transport class is within 10% of the port's cap; red at the cap, where a shipment pays double. Sales: unsold stock and glut in one, since only one applies at a time; stock piling up unsold (the pallet: amber at three turns' output, red at six) when it does, otherwise the sale price against its impact-free base (the coin with a falling arrow: amber 5% or more under it or while the company's own rolling selling is past a rung of the impact ladder, red 10% or more under). A power plant, a battery or infrastructure has all five unlit.
