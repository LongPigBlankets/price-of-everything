...
func _test_recording_toasts() -> void:
	...
	_check(panel.mouse_filter == Control.MOUSE_FILTER_IGNORE and panel.get_child(0).mouse_filter == Control.MOUSE_FILTER_IGNORE,
		"toast rows: a row never intercepts input")
	_check(panel.get_child(panel.get_child_count() - 1) is Label, "toast rows: the row's text is its label")
	panel.free()
	add_child(toast)
	toast.show_caution("Planning")
	toast.show_caution("Planning")
	_check(toast.row_count() == 1, "planning toasts: repeated amber message appears once")
	toast._on_building_added({"building_id": "b_004", "owner": "Three Diamonds Shipping Corporation"})
	_check(toast.row_count() == 1, "building toasts: NPC ports create no notification")
	toast.queue_free()
