...
func _enforce_new_prohibitions() -> void:
	...
	EventScheduler.emit_event({
		...
		"body": "%d %s stopped and %d standing market %s cancelled. Existing shipments will still arrive. No new orders can be placed. Change the affected recipes or demolish the buildings to avoid ongoing costs." % [
...
func _seed_if_needed() -> void:
	...
	EventScheduler.schedule(beat("election_news") - 1, {
		...
		"body": "The Markets Party has won the election. It plans changes to energy and environmental policy.",
		"forewarn_turns": 0,
		"source": "policy",
		"persistent": true,
		"auto_dismiss_turns": 6,
	...
	EventScheduler.schedule(beat("subsidy_end"), {
		...
		"kind": "policy_enacted",
		"severity": "warning",
		"title": "Green Energy Subsidy Ended",
		"body": "Green power no longer earns subsidy payments.",
		"forewarn_turns": 0,
		"source": "policy",
		"deeplink": {"panel": "money"},
		"persistent": true,
...
func _maybe_fire_insider_tip() -> void:
	...
	EventScheduler.emit_event({
		...
		"body": "%s reports that the government is considering a tax on coal and oil use. No policy has been announced yet." % officer_name,
		"source": "policy",
		"persistent": true,
