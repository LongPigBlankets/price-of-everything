	...
	match cmd:
		...
		"toggle":
			...
			if parts.size() >= 2 and parts[1].to_lower() == "midcentury":
				...
				return "map style → %s" % _style_name()
			return "usage: toggle logs | heightmap | roads | roadocc | ink | plate | midcentury | bdp v3 | topbar ds2 | tvp v3 | upgrade ds2"
		"anim":
