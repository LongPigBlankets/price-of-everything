...
func _refresh_preview(amount: float) -> void:
	...
	# Interest runs for the grace turns too, exactly as LoanState._create_loan charges it.
	var term := float(EconomyConfig.LOAN_TERM_TURNS)
	var total_repay: float = amount * (1.0 + LoanState.effective_loan_interest_rate() * (term + float(EconomyConfig.LOAN_GRACE_TURNS)) / term)
