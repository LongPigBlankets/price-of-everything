...
func _on_money_changed(new_amount: float) -> void:
	if _prev_money >= 0.0 and new_amount < 0.0:
		_push_toast(_warning_stack, "!  Cash is in the red: £%.2f" % new_amount, true)
...
func _make_toast(message: String, is_warning: bool) -> PanelContainer:
	var panel := PanelContainer.new()
	panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
	panel.size_flags_horizontal = Control.SIZE_FILL

	var sb := StyleBoxFlat.new()
	sb.bg_color = WARNING_BG if is_warning else SUCCESS_BG
	sb.border_color = WARNING_BORDER if is_warning else SUCCESS_BORDER
	...
	label.add_theme_color_override("font_color", WARNING_TEXT if is_warning else SUCCESS_TEXT)
