...
| Goods bought but never arrive | find the shipment in `TransportState.pending_transport_shipments` — check `destination_tile`, `turns_remaining` | destination full (overflow held + retried — check `get_overflow_shipments_for_tile`); wrong destination; fluid mis-route |
...
| Same seed, different outcomes | run the sim-core RNG gate (`cnc-architecture-contract` §3 filtered grep — must be empty); also suspect new dict-iteration-order dependence or frame-time dependence | determinism break — treat as release-blocking (`cnc-architecture-contract` rule 3) |
