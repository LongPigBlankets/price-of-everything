...
VARIANTS = {                     # owner 2026-08-05: two three-floor, two four-floor
    1: dict(floors=3, wa=4.2, wb=2.6, depth=1.45, mirror=False),
    2: dict(floors=3, wa=3.4, wb=3.1, depth=1.45, mirror=True),
    3: dict(floors=4, wa=5.2, wb=2.1, depth=1.55, mirror=False),
    4: dict(floors=4, wa=3.9, wb=2.6, depth=1.40, mirror=True),
}

# One office floor == one factory floor (owner: "use the annex for proportion" — the
# factory's front annex is exactly one FLOOR = 1.1). At the old 0.62 the offices
# rendered at 56% of factory scale and read as toys next to the plants.
FLOOR_H = 1.1
PARAPET = 0.10
EPS = 0.015
...
def build_office(variant: int = 1) -> dict:
    ...
    # Window and spacing scale with the floor module so the grid keeps its rhythm.
    win_w, win_h = 0.46, 0.60
    for f in range(floors):
        zc = f * FLOOR_H + FLOOR_H * 0.55
        n = max(2, int(wa / 0.95))
        for i in range(n):
            xc = ax0 + (i + 0.5) * wa / n
            if f == 0 and abs(xc) < 0.62:      # ground floor entrance bay stays clear
                continue
            K.window("wa_f%d_%d" % (f, i), "-Y", (xc, 0.0, zc), win_w, win_h, 1, 1)
        for j in range(max(1, int(wb / 1.05))):
    ...
    # it reaches exactly as high as the ground row's window top, no higher).
    door_top = FLOOR_H * 0.55 + win_h / 2
    K.box("door_frame", 0.0, 0.012, (door_top + 0.10) / 2, 0.54, 0.05,
          door_top + 0.10, K.mat("wall_brick"))
    K.box("door", 0.0, -EPS, door_top / 2, 0.40, 0.05, door_top, K.mat("darkmetal"))
