"""Arm FRAMES for the assembly plant (2026-09-10): render_sprite.py's colour + mask passes,
but with ROBOT_POSE injected before the build and every non-arm object of the building made a
HOLDOUT — it punches transparent holes where it is in front of an arm, so the frame laid over
the sprite keeps the pillars' occlusion. Usage:
    blender --background --factory-startup --python render_arms.py -- <builder.py> <fn> <level> <out.png> <BLDG_col> <lift> <carry 0|1>
"""
#!/usr/bin/env python3
...
ns["ROBOT_POSE"] = POSE
ns["ROBOT_ARMS"] = True
print("BUILD", ns[fn_name]() if level == 0 else ns[fn_name](level), "pose", POSE, flush=True)
ARM_NAMES = set(ns["ROBOT_NAMES"])
print("ARM OBJECTS", len(ARM_NAMES), flush=True)
...
for ob in mine_col.objects:
    ...
    if ob.name not in ARM_NAMES:
        ob.is_holdout = True          # cuts the arm behind it; renders nothing itself
# Ink only the arms: Freestyle keeps the whole scene in its view map (so a pillar still hides
# the arm edges behind it) but draws lines only for the ARMS collection.
arms_col = bpy.data.collections.new("ARMS")
scene.collection.children.link(arms_col)
for ob in mine_col.objects:
    if ob.name in ARM_NAMES:
        arms_col.objects.link(ob)
for ls in scene.view_layers[0].freestyle_settings.linesets:
    ls.select_by_collection = True
    ls.collection = arms_col
print("ARMS collection", len(arms_col.objects), flush=True)
