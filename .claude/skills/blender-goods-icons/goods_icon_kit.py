...
# ---------------------------------------------------------------- IRON ORE
def nugget(K, name, centre, r, seed, squash=(1.0, 0.92, 0.80), mats=(None, None, None), cuts=2, noise_amp=0.11, subdiv=2, planes=8, rust_depth=(0.60, 0.74), crack_range=(2, 4), grey_depth=(0.74, 0.83), subsurf_levels=2, crease=0.7, taper=0.0):
    ...
    if taper != 0.0:
        # narrow the TOP so the mass sits low (owner: bottom-heavy). t=0 at the base, 1 at the crown
        zs = [v.co.z for v in bm.verts]
        zlo, zhi = min(zs), max(zs)
        for v in bm.verts:
            t = (v.co.z - zlo) / max(1e-6, zhi - zlo)
            s = 1.0 - taper * t
            v.co.x *= s; v.co.y *= s
...
def build_coal():
    ...
    # CHUNKIER blocks (owner): fewer, deeper cuts -> a few big blocky faces per chunk, more
    # equant (not tall/lumpy). (centre, r, seed, shine caps, planes, squash, dark cut depth)
    lumps = [
        (( 0.05,  0.32), 1.02, 41, 1, 5, (1.02, 0.94, 1.02), (0.46, 0.62)),  # big central block
        ((-0.66, -0.26), 0.62, 42, 1, 5, (1.02, 0.94, 0.96), (0.44, 0.60)),  # left block
        (( 0.70, -0.30), 0.64, 43, 1, 5, (1.02, 0.94, 0.98), (0.44, 0.60)),  # right block
    ]
    for i, ((cx, cy), r, seed, cuts, planes, sq, rd) in enumerate(lumps):
        cz = r * sq[2] * 0.72
        gd = (0.88, 0.93)      # SMALL glint facets, not big silver caps
        nugget(K, "chunk%d" % i, (cx, cy, cz), r, seed, squash=sq, mats=(coal, shine, shine_hi),
               ...
               noise_amp=0.05, subsurf_levels=1, crease=1.0)
...
def build_crude_oil():
    ...
    K = Kit(col)
    steel = toon_mat("tn_drum", (0.055, 0.062, 0.072))    # CHARCOAL-black drum (owner: not blue)
    steel_dk = toon_mat("tn_drum_dk", (0.026, 0.030, 0.036))
    lid = toon_mat("tn_lid", (0.13, 0.14, 0.16))          # lighter grey lid so the black oil reads
    oil = toon_mat("tn_oil", (0.012, 0.014, 0.020))
    R, H = 0.52, 1.52
    ...
    for i, zz in enumerate((0.02, -0.42)):        # two body hoops (owner: removed the top ring)
        K.washer("hoop%d" % i, (0, 0, zz), (0.0, 0.0, 1.0), R * 0.99, R * 1.05, 0.055, steel_dk, seg=48)
    # a LIGHTER grey lid disc (so the black oil reads on it), then a big irregular LOBED oil
    # puddle (owner: more like the good icon's splash - covers most of the lid, lobes reaching
    # between the bungs) and two bung rings
    K.cyl("lid", 0, 0, H / 2 + 0.005, R * 0.96, 0.02, lid, axis='Z', segments=48, smooth=True)
    blob(K, "oil", (0.03, -0.02, H / 2 + 0.03), 0.27, 61, (1.2, 1.0, 0.05), oil, noise_amp=0.5)
    # a bright REFLECTION streak on the oil (owner: more reflective) - a light sheen toward the
    # lit (front) side, sitting just proud of the puddle
    oil_shine = toon_mat("tn_oil_shine", (0.34, 0.40, 0.50), steps=((0.66, 0.9), (0.92, 0.95), (9.0, 1.0)))
    blob(K, "oil_gloss", (-0.02, -0.13, H / 2 + 0.055), 0.10, 63, (1.7, 0.7, 0.05), oil_shine, noise_amp=0.35)
    K.washer("bung0", (0.33, 0.18, H / 2 + 0.03), (0.0, 0.0, 1.0), 0.045, 0.085, 0.05, steel_dk, seg=20)
    K.washer("bung1", (-0.27, -0.24, H / 2 + 0.03), (0.0, 0.0, 1.0), 0.045, 0.085, 0.05, steel_dk, seg=20)
    return {"objects": len(col.objects)}


def mottle_rock(K, name, centre, r, seed, squash, mats, patch_scale=1.8, patch_bias=0.10,
                noise_amp=0.13, subsurf_levels=2, dissolve_ang=10.0):
    ...
    bmesh.ops.recalc_face_normals(bm, faces=bm.faces)
    bmesh.ops.dissolve_limit(bm, angle_limit=math.radians(dissolve_ang),
...
def build_copper_ore():
    ...
    # finer, more GRANULAR green speckle (owner) -> higher patch_scale, less patch-merging
    mottle_rock(K, "rock", (0.0, 0.0, 0.86), 1.05, 71, (1.14, 0.93, 0.88), (green, brown),
                patch_scale=5.0, patch_bias=0.04, noise_amp=0.22, dissolve_ang=5.0)
    return {"objects": len(col.objects)}
...
def build_bauxite_ore():
    ...
    K = Kit(col)
    red = toon_mat("tn_bauxite", (0.46, 0.18, 0.07))
    cutm = toon_mat("tn_bauxite_cut", (0.56, 0.28, 0.14))    # lighter interior on the sliced face
    dotm = toon_mat("tn_bauxite_dot", (0.055, 0.028, 0.016)) # near-black pellets inside (owner)
    rng = random.Random(91)
    R, sx, sy, sz = 1.0, 1.14, 1.06, 1.02
    # SUB-OBJECTS (owner): full pebbles on the outside; the SLICE cuts through the ones at the edge
    # so they become HALF-SPHERES at the rim; and dark near-black PELLETS scattered on the flat
    # interior. Cut normal = camera direction so the flat cross-section face is toward us.
    cn = Vector((0.577, -0.577, 0.577)).normalized()
    bm = bmesh.new()
    tmp = bmesh.new(); bmesh.ops.create_icosphere(tmp, subdivisions=3, radius=R * 0.92)
    for v in tmp.verts:
        v.co = Vector((v.co.x * sx, v.co.y * sy, v.co.z * sz))
    m0 = bpy.data.meshes.new("_b"); tmp.to_mesh(m0); tmp.free(); bm.from_mesh(m0)
    # full pebbles on the whole visible surface, added BEFORE the slice so the rim ones get cut.
    # Fewer + larger -> cleaner distinct half-spheres at the rim (240 small ones tangled)
    N = 150
    ga = _m.pi * (3 - _m.sqrt(5))
    for i in range(N):
        yy = 1 - (i / (N - 1)) * 2
        rad = _m.sqrt(max(0.0, 1 - yy * yy)); th = i * ga
        pos = Vector((_m.cos(th) * rad * R * sx, yy * R * sy, _m.sin(th) * rad * R * sz))
        if pos.normalized().dot(Vector((0.35, -0.5, 0.72)).normalized()) < -0.35:
            continue                                   # deep back, never seen
        nr = R * rng.uniform(0.17, 0.23)
        nb = bmesh.new(); bmesh.ops.create_icosphere(nb, subdivisions=3, radius=nr)
        bmesh.ops.translate(nb, verts=nb.verts, vec=pos * 0.82)
        nm = bpy.data.meshes.new("_n%d" % i); nb.to_mesh(nm); nb.free(); bm.from_mesh(nm)
    for f in bm.faces:
        f.material_index = 0
    # SLICE base + pebbles together (keep the far half): the near pebbles vanish, the pebbles the
    # plane passes through become HALF-SPHERES at the rim, the base interior becomes the flat face.
    ext = max(v.co.dot(cn) for v in bm.verts)
    cut_co = cn * ext * 0.22
    res = bmesh.ops.bisect_plane(bm, geom=bm.verts[:] + bm.edges[:] + bm.faces[:],
                                 plane_co=cut_co, plane_no=cn, clear_outer=True, clear_inner=False)
    edges = [g for g in res["geom_cut"] if isinstance(g, bmesh.types.BMEdge)]
    fill = bmesh.ops.holes_fill(bm, edges=edges, sides=0)
    for f in fill["faces"]:
        f.material_index = 1
    bm.normal_update()
    # dark near-black PELLETS on the CLEAN interior (clustered toward the centre, away from the
    # rim's half-spheres). Flat coplanar circles -> consistent lighting with the cut face.
    up = Vector((0, 0, 1)); u = cn.cross(up).normalized(); w = u.cross(cn).normalized()
    rot = Vector((0, 0, 1)).rotation_difference(cn).to_matrix().to_4x4()
    for k in range(46):
        rr = R * 0.42 * (rng.random() ** 0.6); ang = rng.uniform(0, 2 * _m.pi)
        p = cut_co + u * (rr * _m.cos(ang)) + w * (rr * _m.sin(ang) * 1.2) + cn * 0.006
        db = bmesh.new()
        bmesh.ops.create_circle(db, cap_ends=True, segments=14, radius=R * rng.uniform(0.028, 0.048))
        bmesh.ops.transform(db, matrix=rot, verts=db.verts)
        bmesh.ops.translate(db, verts=db.verts, vec=p)
        for f in db.faces:
            f.material_index = 2
        dm = bpy.data.meshes.new("_d%d" % k); db.to_mesh(dm); db.free(); bm.from_mesh(dm)
    for v in bm.verts:
        v.co = v.co + Vector((0.0, 0.0, R * sz * 0.86))
    me = bpy.data.meshes.new("bauxite")
    bm.to_mesh(me); bm.free()
    ob = K.obj("bauxite", me, red)
    ob.data.materials.append(cutm); ob.data.materials.append(dotm)
    for p in ob.data.polygons:
        p.use_smooth = True
    # the pea-cross-section DOTS (material 2) get NO ink outline (owner: thinner/disappear) - they
    # read by tone alone. `ink` excludes freestyle_face-marked faces (EXCLUSIVE, condition ONE).
    ff = me.attributes.get("freestyle_face") or me.attributes.new("freestyle_face", 'BOOLEAN', 'FACE')
    for i, p in enumerate(me.polygons):
        ff.data[i].value = (p.material_index == 2)
    return {"objects": len(col.objects)}


def sweep_tube(K, name, path, tube_r, mat, seg=12):
    """Sweep a circle of radius `tube_r` along a centreline `path` (list of Vectors) into a smooth
    tube - a horseshoe magnet, a pipe, a handle. Capped ends, smooth-shaded."""
    import bmesh
    from mathutils import Vector
    bm = bmesh.new()
    rings = []
    ...
    for i, p in enumerate(path):
        t = (path[1] - path[0]) if i == 0 else (path[-1] - path[-2]) if i == n - 1 else (path[i + 1] - path[i - 1])
        t = t.normalized()
        ref = Vector((0, 1, 0)) if abs(t.y) < 0.9 else Vector((1, 0, 0))
        a = t.cross(ref).normalized(); b = t.cross(a).normalized()
        ring = [bm.verts.new(p + a * (tube_r * math.cos(2 * math.pi * k / seg)) + b * (tube_r * math.sin(2 * math.pi * k / seg))) for k in range(seg)]
        rings.append(ring)
...
def build_ree_ore():
    """Rare Earth Ore (owner rebuild): the shipped SYMBOL - a red horseshoe MAGNET (rare earths
    make the strongest magnets) in front of a cluster of different mineral CLUMPS: amber monazite,
    silvery SCANDIUM (a distinct metal, was missing), pink and teal REE oxides. Reference:
    g_032_ree_ore.png."""
    from mathutils import Vector
    setup_icon_rig()
    fs = _ore_linestyle()
    ...
    K = Kit(col)
    amber = toon_mat("tn_ree_amber", (0.60, 0.34, 0.07))
    scandium = toon_mat("tn_ree_sc", (0.52, 0.55, 0.60))       # silvery scandium metal
    pink = toon_mat("tn_ree_pink", (0.52, 0.24, 0.34))
    teal = toon_mat("tn_ree_teal", (0.10, 0.34, 0.32))
    magnet = toon_mat("tn_ree_magnet", (0.56, 0.06, 0.06))     # red magnet body
    pole = toon_mat("tn_ree_pole", (0.66, 0.68, 0.72))         # silver pole tips
    # mineral clumps STANDING behind (each a solid-colour nugget)
    clumps = [
        ((-0.34,  0.44), 0.58, 101, (1.0, 0.92, 0.98), amber, 0.20, 9),     # amber, spiky
        (( 0.30,  0.52), 0.50, 102, (1.0, 0.92, 1.02), scandium, 0.08, 7),  # silvery scandium
        (( 0.64,  0.12), 0.46, 103, (1.0, 0.92, 0.98), pink, 0.10, 7),      # pink oxide
        ((-0.66,  0.06), 0.44, 104, (1.0, 0.92, 0.98), teal, 0.12, 7),      # teal oxide
    ]
    for i, ((cx, cy), r, seed, sq, m, na, pl) in enumerate(clumps):
        cz = r * sq[2] * 0.66
        nugget(K, "clump%d" % i, (cx, cy, cz), r, seed, squash=sq, mats=(m, m, m),
               cuts=0, planes=pl, rust_depth=(0.56, 0.72), crack_range=(0, 0),
               noise_amp=na, subsurf_levels=1, crease=0.92)
    # the RED horseshoe magnet lying FLAT ON THE FLOOR in front (owner), opening toward the camera
    w, tr = 0.42, 0.15
    z0 = tr                                       # resting on the ground
    y_front, y_back = -0.78, 0.02
    path = []
    for j in range(6):
        path.append(Vector((w, y_front + (y_back - y_front) * j / 5, z0)))
    for j in range(1, 13):
        phi = math.pi * j / 12
        path.append(Vector((w * math.cos(phi), y_back + w * math.sin(phi), z0)))
    for j in range(6):
        path.append(Vector((-w, y_back + (y_front - y_back) * j / 5, z0)))
    sweep_tube(K, "magnet", path, tr, magnet, seg=14)
    # silver pole tips at the two leg FRONT ends (facing the camera)
    for sgn in (-1, 1):
        K.cyl("pole%d" % (sgn > 0), sgn * w, y_front - 0.02, z0, tr * 1.02, 0.16, pole, axis='Y', segments=14, smooth=True)
    return {"objects": len(col.objects)}
...
def build_lithium_ore():
    ...
    setup_icon_rig()
    fs = _ore_linestyle()
    # WHITE cleavage VEINS (owner: the good icon's white striations): interior cleavage lines are
    # drawn light, and the navy silhouette (2D contour + `ink` silhouette) still frames the crystal
    fs.linesets["ink_edge"].linestyle.color = (0.82, 0.74, 0.80)
    fs.linesets["ink_edge"].linestyle.thickness = 4.5
    fs.linesets["ink"].select_crease = False      # so interior facets aren't inked navy
    ...
    # ONE single WIDE, JAGGED crystal rock (owner: the good icon is a single wide jagged rock, not
    # a tall multi-nugget cluster). Wide squash, higher noise + more planes for jaggedness.
    nugget(K, "xtal", (0.0, -0.05, 0.66), 1.18, 81, squash=(1.28, 1.05, 0.80),
           mats=(pink, pink_hi, pink_hi2), cuts=2, planes=8, rust_depth=(0.56, 0.74),
           crack_range=(0, 0), grey_depth=(0.66, 0.78), noise_amp=0.15, subsurf_levels=1, crease=1.0)
    return {"objects": len(col.objects)}
...
def build_alloy_ore():
    ...
    K = Kit(col)
    # (centre, r, seed, squash, base, taper, planes, noise) - cuts=0 so each rock is one metal
    lumps = [
        (( 0.10,  0.28), 0.94, 51, (1.02, 0.94, 1.06), (0.30, 0.33, 0.38), 0.46, 8, 0.08),  # slate hero, BOTTOM-heavy
        ((-0.60, -0.16), 0.60, 52, (1.02, 0.94, 0.98), (0.10, 0.115, 0.14), 0.0, 11, 0.22),  # dark steel, SPIKY
        (( 0.02, -0.55), 0.58, 53, (1.00, 0.95, 0.90), (0.52, 0.42, 0.16), 0.0, 9, 0.10),    # brass / gold
        (( 0.66, -0.40), 0.58, 54, (1.00, 0.92, 0.95), (0.56, 0.59, 0.64), 0.0, 8, 0.09),    # bright silver
    ]
    for i, ((cx, cy), r, seed, sq, base, tp, pl, na) in enumerate(lumps):
        cz = r * sq[2] * 0.72
        m = toon_mat("tn_alloy%d" % i, base)
        nugget(K, "rock%d" % i, (cx, cy, cz), r, seed, squash=sq, mats=(m, m, m),
               cuts=0, planes=pl, rust_depth=(0.56, 0.72), crack_range=(0, 0),
               noise_amp=na, subsurf_levels=1, crease=0.94, taper=tp)
    return {"objects": len(col.objects)}
